...
class Fetcher(ABC):
    ...
    @abstractmethod
    def fetch(self, symbols: List[str], start: datetime = None, end: datetime = None, interval: str="1d") -> DataFrame:
        pass   
...
class YahooFetcher(Fetcher):
    ...
    def fetch(self, symbols: List[str], start: datetime = None, end: datetime = None, interval: str="1d") -> DataFrame:
        logger.info(f"[fetch|in] ({symbols}, {start}, {end})")
        ...
        if not market_data.empty:
            ...
            for key, val in market_data.to_dict().items():
                ...
                for ts, num in val.items():
                    if not isnull(num):
                        entry = DataFrame({"symbol": symbol, "variable": [variable], "value": [num], "actual_time": [ts.timestamp()]})
